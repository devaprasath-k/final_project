...
# ═════════════════════════════════════════════════════════════════════════════
# ✅ AUTO-LOGIN FROM COOKIE - Check BEFORE setting any session state
# ═════════════════════════════════════════════════════════════════════════════
if "logged_in" not in st.session_state:
    ...
    if st.session_state.get("just_logged_out"):
        ...
        if user_id_cookie and str(user_id_cookie).strip():
            try:
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
                c.execute('SELECT * FROM users WHERE user_id = ?', (int(user_id_cookie),))
                user = c.fetchone()
                conn.close()
                if user:
    ...
    with col_left:
        ...
        if st.button("🚀 Login", key="login_btn", use_container_width=True):
            ...
            if user:
                ...
                st.session_state.current_page = "Dashboard"
                st.session_state.just_logged_out = False  # ✅ ADD THIS LINE
...
def main():
    ...
    # ✅ Set flag FIRST
                    ...
                    # Reset keys individually (NOT clear())
                    st.session_state.logged_in = False
                    st.session_state.user_id = None
                    st.session_state.user_data = {}
                    st.session_state.current_page = "Login"
